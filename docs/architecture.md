...
Transcript locate is local-only. It searches cached word timings, and optional `--clip-out` extraction shells out to `ffmpeg` with stream copy instead of decoding or re-encoding audio in Go.
